...
{
public:
    StateChangeObserver(std::shared_ptr<Chart>& c):chart(c){}
...
struct SubChartControl
{
    ...
    std::shared_ptr<Chart> createSubChart() {
        ...
        s1->createEventCallback(e2,
                std::bind(&StateEventLogger::callback, &s1EventLogger,
                    std::placeholders::_1));
...
int main(void)
{
    ...
    auto tSub = state1->createTransition(subchart,
            [](){std::cout<<"[tSub]"<<std::endl;});
    auto tFinal = subchart->createTransition(chart->getFinalState(),
            [](){std::cout<<"[tFinal]"<<std::endl;});
    auto tAgain = subchart->createTransition(state1,
    ...
    Event eSub{"eSub"};
    subchart->createEventCallback(eSub,
            /* let's reuse the one in our subchart */
            std::bind(&StateEventLogger::callback, &subChartControl.s1EventLogger,
                std::placeholders::_1));
