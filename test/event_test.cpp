...
public:
  ...
  const std::string & getState1Name() override {return getChartName();}
  const std::string & getState2Name() override {return dummy2Name;}
...
TYPED_TEST_CASE(EventTest, Implementation);
...
TYPED_TEST(EventTest, spinOnce)
{
  ...
  c.chart->getInitialState()->createEventCallback(
    eInit,
  ...
  c.chart->getFinalState()->createEventCallback(
    eFinal,
    [&eventFiredName, &eventFiredCount](auto e)
    {
      eventFiredName = e.name();
...
public:
  void SetUp() override
  {
    ...
    chart->getInitialState()->createEventCallback(
      eInit,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));

    state1->createEventCallback(
      eState1,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));

    state2->createEventCallback(
      eState2,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));

    chart->getFinalState()->createEventCallback(
      eFinal,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));
    ...
    subchart->getInitialState()->createEventCallback(
      eSubInit,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));

    subState1->createEventCallback(
      eSubState1,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));

    subState2->createEventCallback(
      eSubState2,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));

    subchart->getFinalState()->createEventCallback(
      eSubFinal,
      std::bind(
        &EventLogger::eventCallback, &eventLogger,
        std::placeholders::_1));
